...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, onNavigateToRequests, onNavigateToFieldRequest, onNavigateToReports, onNavigateToLogin, onNavigateToDashboards, activeView, hideOnDesktop = false }) => {
  ...
  const getActiveViewFromRoute = (): 'list' | 'create' | 'fieldRequest' | 'reports' | 'dashboards' => {
  ...
  const currentActiveView = activeView || getActiveViewFromRoute();
  const isRequestsActive = currentActiveView === 'list' || currentActiveView === 'create';
  ...
  const handleFieldRequestClick = () => {
    navigate(getAreaPath('field-request'));
  ...
  const handleDashboardsClick = () => {
    navigate(getAreaPath('dashboards'));
